...
def LoginPanel():
    ...
    if res.text.find('/logout/') != -1:
        ...
        print('面板登录失败')
        send_telegram_message("面板登录bb1失败")  # 发送电报通知
        os._exit(0)
...
def LoginSsh():
    ...
    with paramiko.SSHClient() as ssh:
        ...
        try:
            ...
            print(f"当前IP: {ip}")
            send_telegram_message(f"serv00连接成功bb1，当前IP: {ip}")  # 发送电报通知
        except Exception as e:
            print(f"SSH连接失败: {e}")
            send_telegram_message(f"SSH连接失败bb1: {e}")  # 发送电报通知
            os._exit(0)

if __name__ == '__main__':
    print("启动脚本")
    # 从环境变量中获取服务器信息
    Serv00 = os.getenv("LOGIN_URL1")
    if Serv00 is None:
